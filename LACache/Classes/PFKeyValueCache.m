...
@property (nullable, atomic, readwrite, copy) id<NSCoding> value;
...
- (instancetype)initWithValue:(id<NSCoding>)value creationTime:(NSDate *)creationTime {
    self = [super init];
    if (!self) return nil;
...
+ (instancetype)shareInstance{
    static PFKeyValueCache *instance;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        NSString *rootPath = [NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) objectAtIndex:0];
        instance = [[self alloc] initWithCacheDirectoryPath:[rootPath stringByAppendingPathComponent:@"com.laker.keyvaluecache"]];
    });
    return instance;
}
...
- (instancetype)initWithCacheDirectoryURL:(NSURL *)url
    ...
    _diskCacheQueue = dispatch_queue_create("com.laker.keyvaluecache.disk", DISPATCH_QUEUE_SERIAL);
...
- (void)setObject:(id <NSCoding>)value forKey:(NSString *)key {
    ...
    NSData *bytes = [NSKeyedArchiver archivedDataWithRootObject:value];
    NSUInteger valueBytes = [bytes length];
    ...
    dispatch_async(_diskCacheQueue, ^{
        [self _createDiskCacheEntry:bytes atURL:[self _cacheURLForKey:key]];
        [self _compactDiskCache];
    });
}

- (id <NSCoding>)objectForKey:(NSString *)key{
    return [self objectForKey:key maxAge:60 * 60 * 24 * 30];
}



- (nullable id<NSCoding>)objectForKey:(NSString *)key maxAge:(NSTimeInterval)maxAge {
    key = [key LA_md5];
...
- (id<NSCoding>)_diskCacheEntryForURL:(NSURL *)url {
    ...
    return [NSKeyedUnarchiver unarchiveObjectWithData:bytes];
}

- (void)_createDiskCacheEntry:(NSData *)bytes atURL:(NSURL *)url {
